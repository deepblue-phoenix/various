...
def run_task_on_node_phys(p_task_fun,
	...
	#all_hosts_return_vals_dict - :Dict<key:String(host_str),:String(task_std_out_str)>
	#                             a dict of standard outputs of each task run, 
	#                             per host supplied in "hosts" argument
	all_hosts_return_vals_dict = p_fab_api.execute(p_task_fun,
												hosts = fabric_format_hosts_lst)
	assert isinstance(all_hosts_return_vals_dict,dict)
	#-------------------
	...
	if p_verbose_bool == True:
		for host_str,host_std_output_str in all_hosts_return_vals_dict.items():
			p_log_fun('INFO','************************************-------------------------------')
